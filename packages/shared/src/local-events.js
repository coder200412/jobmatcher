...
async function buildJobAlertNotifications(payload) {
  ...
  const ranked = candidatesResult.rows
    .map((candidate) => {
      ...
      const matchPercent = computeCandidateAlertMatch(candidate, payload);
      return {
        candidate,
        matchPercent,
        matchedSkills,
  ...
  const notifications = [
    {
      ...
      message: `Your job "${event.payload.title}" at ${event.payload.company} is now live.`,
      data: {
        jobId: event.payload.jobId,
        priorityScore: event.payload.priorityScore || 0,
      },
    },
    ...(await buildJobAlertNotifications(event.payload)),
  ];
